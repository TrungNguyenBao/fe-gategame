...
export function Button({
  ...
  const finalClassName =
    `${buttonClass} ${buttonText} ${buttonHover} ${buttonSize} ${props.unfocusable ? 'no-focus' : ''
      } ${iconPosition == 'end' ? 'flex-row-reverse' : ''} ${loading ? 'loading' : ''
      } ${className}`.trim()
    ...
    <>
      ...
      {props.text && (
        <span
          ...
          className={`relative transform transition-transform w-full inline-block ${!props.icon && loading ? 'translate-x-2.5' : ''
            }`}
        >
          {!props.icon && loading && (
